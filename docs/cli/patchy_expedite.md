...
Expedite a finding past the accumulation window and the queue
...
Your RBAC decides whether it lands: the CLI checks the 'expedite' verb before
writing, and the cluster's admission policy enforces it regardless of client.

```
patchy expedite finding <name...> [flags]
```
...
```
  ...
  patchy expedite finding -l patchy.bitwisemedia.uk/severity=critical
  patchy expedite finding my-finding --dry-run
```
