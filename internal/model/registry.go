...
// harness's --model flag expects. patchy's own harnesses are vendor-native
// (Claude Code drives Anthropic models, Codex drives OpenAI models), so each
// model names its vendor's harness as Preferred; Copilot is the exception the
// map shape was built for — it brokers both vendors, so it appears as a second
// supported entry on every model rather than as any model's preferred harness.
//
// Copilot's ids diverge from the vendors' own where the vendor uses a dashed
// point release (claude-haiku-4-5 is claude-haiku-4.5 on Copilot), which is
// exactly the divergence Supported exists to record. Copilot brokers models
// beyond these; only the models patchy already lists are registered, so
// enabling the copilot runner widens which harness can run a model, never
// which models exist.
func builtins() []Model {
	return []Model{
		...
		// OpenAI — driven by Codex, all also by Copilot. Neither CLI reports a
		// cost of its own, so unlike the Anthropic rates (a cross-check on a
		// figure the claude harness already reports) these are the only thing
		// that prices a codex or copilot run at all — see agentresult.stageCost.
		// The GPT-5.6 family is three named tiers rather than a size suffix:
		// Luna is the nano-like tier, Terra the mini-like one, Sol the flagship.
		{
			...
			Supported: map[string]string{HarnessCodex: "gpt-5.3-codex", HarnessCopilot: "gpt-5.3-codex"},
...
var KnownHarnessIDs = []string{HarnessClaude, HarnessCodex, HarnessCopilot, HarnessFake}
