...
type forgeWriter struct {
	forges *forge.Store
	pusher *ghpush.Pusher
}

// NewForgeWriter builds the production ForgeWriter over the shared forge
// store and the Git Data API pusher.
func NewForgeWriter(forges *forge.Store, pusher *ghpush.Pusher) ForgeWriter {
	return &forgeWriter{forges: forges, pusher: pusher}
}

// Push implements ForgeWriter.
func (w *forgeWriter) Push(ctx context.Context, namespace, repoURL, branch string, cs *envelope.Changeset) error {
	...
	return w.pusher.Push(ctx, res.Repo, token, branch, cs)
}
